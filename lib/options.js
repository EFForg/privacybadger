...
function getOriginsArray()
{
  ...
  // Convert origins object to array.
  var originsArray = [];
  for (var origin in origins) {
    originsArray.push(origin);
  }

  // Update origin count on options page.
  $("#count").text(originsArray.length);
...
function showTrackingDomains(domains) {
  ...
  var domainActions = getOrigins();
  var trackingDetails = '<div id="blockedResourcesInner" class="clickerContainer">';
  for (var i = 0; i < domains.length; i++) {
    var trackingDomain = domains[i];
    // todo: gross hack, use templating framework
    var action = optionsGetAction(trackingDomain, domainActions);
    if (action) {
      trackingDetails = _addOriginHTML(trackingDomain, trackingDetails, action);
    }
  }
  trackingDetails += '</div>';
