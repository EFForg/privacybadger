 ...
 *
 * You should have reccentralizeeived a copy of the GNU General Public License
 * along with Adblock Plus.  If not, see <http://www.gnu.org/licenses/>.
 */
var idOfDeleteButton = "deleteButton";
...
function _addOriginHTML(origin, printable, action) {
  ...
  var item =  printable + '<div id="domainDiv_' + cnt + '"' + classText + '" data-origin="' + origin + '" tooltip="' + _badgerStatusTitle(action) + '" data-original-action="' + action + '"><div class="origin" >' + _trim(origin,30) + '</div>' + _addToggleHtml(origin, action) + '<button class= "deleteButtonClass" id=id_'+cnt+'>X</button> <div class="honeybadgerPowered tooltip" tooltip="'+ feedTheBadgerTitle + '"></div><img class="tooltipArrow" src="/icons/badger-tb-arrow.png"><div class="tooltipContainer"></div></div>';
    //console.log(cnt);
    cnt ++;
    return item;
}
//This function runs at refresh automatically, something quite strange
  ...
  $("#blockedResources").html(printable);
    ...
    for(var i = 0; i < cnt ; i++){
        ...
        $("#"+idd).bind('click', function() {
            ...
            var tmpId = someId.split('_');
            
            var finalId = tmpId[1];
            
            var idOfHiddenDiv = 'domainDiv_'+finalId;
            $('#'+idOfHiddenDiv).remove();
             console.log(someId);
//             alert('a die ohhhhh'+ idd);
        });
     }
...
document.addEventListener('click', function(){
    document.getElementByClassName(".deleteButtonClass").onclick = hideDiv();
    idOfDeleteButton = 'deleteButton';

});*/
function hideDiv(){
    //document.getElementById("domainDiv").setAttribute('hidden', 'true');
    alert(this.id);
}
