  ...
  ActiveMatchers.prototype =
  {
    ...
    computeActiveMatchersFromFilters: function(tabId, origin)
    {
      ...
      for (var matcherKey in matcherStore.combinedMatcherStore)
      {
        ...
        // TODO: subsequent subscriptions are overwriting prior ones
        // order is whitelist, hueristic, red, yellow, green
        // if red gets caught, then and yellow parent shows up
        // red gets overwritten, not added to unfired matchers
        // and it shows up as a result with the parent color
        //
        // the fix is to do some check to see if there's an exact match
        // if there is overwrite the parent one
        // but how to add it to unfired matchers??
        // what if before, what if after?
        if (currentFilter && !perfectMatch)
        {
