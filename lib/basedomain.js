 ...
 * name for the document, and the cross domain list to determine whether the
 * document and request are from the same party.
...
function isThirdParty(/**String*/ requestHost, /**String*/ documentHost)
{
  ...
  if (requestHost.length > documentDomain.length) {
    var requestDomain = getBaseDomain(requestHost);
    if (isMultiDomainFirstParty(requestHost, documentDomain)) {
      return false;
    } else {
...
/**
 * Reference the crossDomain list to see if the request and document are from
 * two distinct domains belonging to the same first party.
 * @param requestHost The host of the 3rd party request
 * @param documentDomain The domain of the document
 */
function isMultiDomainFirstParty(/**String*/ requestHost, /**String*/ documentDomain) {
  var requestDomain = getBaseDomain(requestHost);

  return (crossDomain[requestDomain] === crossDomain[documentDomain]);
}
