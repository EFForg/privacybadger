...
require.scopes.webrequest = (function () {
  ...
  function onBeforeSendHeaders(details) {
    ...
    if (_isTabChromeInternal(tab_id)) {
      ...
      if (
        type == "xmlhttprequest" &&
        url.endsWith("/.well-known/dnt-policy.txt")
      ) {
    ...
    if (
      ...
      if (details.method == "GET") {
        newHeaders = details.requestHeaders
          .filter((header) => {
  ...
  function hideBlockedFrame(tab_id, parent_frame_id, frame_url, frame_host) {
    ...
    if (badger.isWidgetReplacementEnabled()) {
      let exceptions = badger
        .getSettings()
        .getItem("widgetReplacementExceptions");
          ...
          if (domain[0] == "*") {
            ...
            return (
              ...
              matches.every((origin) => {
                ...
                return (
                  action == constants.BLOCK || action == constants.USER_BLOCK
                );
              })
            );
          }
  ...
  function dispatcher(request, sender, sendResponse) {
    ...
    if (!sender.url.startsWith(chrome.runtime.getURL(""))) {
      ...
      if (!KNOWN_CONTENT_SCRIPT_MESSAGES.includes(request.type)) {
        console.error(
          "Rejected unknown message %o from %s",
          request,
          sender.url
        );
        return sendResponse();
    ...
    switch (request.type) {
      ...
      case "getBlockedFrameUrls": {
        ...
        let tab_id = sender.tab.id,
          ...
          tabData =
            badger.tabData.hasOwnProperty(tab_id) && badger.tabData[tab_id],
          blockedFrameUrls =
            tabData &&
      ...
      case "getPopupData": {
        ...
        sendResponse({
          ...
          showLearningPrompt: badger
            .getPrivateSettings()
            .getItem("showLearningPrompt"),
          showNonTrackingDomains: badger
            .getSettings()
            .getItem("showNonTrackingDomains"),
          tabHost: tab_host,
          tabId: tab_id,
